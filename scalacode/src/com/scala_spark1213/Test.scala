...
//    setTest()
  ...
  def arrTest():Unit={
    ...
    var numbers = Array(1, 2, 3, 4) //声明一个数组对象
    var r = numbers.drop(0)
    println("r*******")
    for(i <- r) println(i)
    println("drop: " + numbers.length +" r = "+ r.length)
...
//    val t = (1, "hello", Console)
  ...
  def listTest(): Unit ={
    ...
    println("hashCode: " + list2.hashCode())

    val x = list.head
    val y = list(3)
    val z = list.take(3) // List(1,2,3)
    val z2 = list.apply(3)
    val min = list.max
    val max = list.min
  ...
  def setTest(): Unit ={
    ...
    mutableSet.add(4)
    println("length mutableSet: " + mutableSet.size)
...
//    println("length mu : " + A.size)
//    A += ("xxx" -> "zhangshan")
//    A += ("xx2" -> "w1")
//    println("length mu2 : " + A.size)
//    A += ("xx3" -> "r3")
//
//    println(A)
